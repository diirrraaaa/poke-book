...
const url = `https://pokeapi.co/api/v2/pokemon/dratini/`;
 fetch(url)
   .then(res => res.json())
   .then(pokemon => {
...
     let pikachu = new Pokemon(attack,id,Speed,hp,atk,def,specDef,height,weight,anti,adap,spec)
     pikachu.display()
     Nadira = new Trainer()
     Nadira.add(pikachu)
     console.log(pokemon);
   });
   class Pokemon{
     constructor(attack,id,Speed,hp,atk,def,specDef,height,weight,anti,adap,spec){
       this.attack = attack
       this.id = id
       this.Speed = Speed
       this.hp = hp
